...
import { getAllData, getDataKeyboard } from "@/api/product/index.api";
import PaginationProduct from "../pagination";

export default async function AdminProductKeyboard({
  searchParams,
}: {
  searchParams: { page: number };
}) {
  const data = await getDataKeyboard(10, searchParams.page);
  return (
    <main className="">
      <nav
      ...
      >
        <div className="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
          <nav>
            ...
            <h6 className="mb-0 font-bold capitalize">Bàn phím</h6>
          </nav>
        </div>
      </nav>
